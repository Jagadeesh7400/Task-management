...
export default function TaskCard({ task, onTaskComplete, className, style }) {
  ...
  return (
    <div className="task-card">
      ...
      <p className="task-status">Status: {task.status}</p> {/* Display current status */}
      ...
      style={style}
      >
        <div className="task-footer p-4">
          <div className="flex justify-between items-start mb-2">
            <h3 className={`task-title font-semibold text-dark-color dark:text-light-color ${task.status === "completed" ? "line-through opacity-70" : ""}`}>
              {task.title}
            </h3>
          ...
          <p className={`text-sm text-dark-color dark:text-light-color opacity-80 mb-3 ${task.status === "completed" ? "line-through opacity-50" : ""}`}>
            {task.description}
          </p>
